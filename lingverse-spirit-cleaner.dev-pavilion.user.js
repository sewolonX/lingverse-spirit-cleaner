...
(function () {
    ...
    // --- 珍宝阁 ---
    async function fetchPavilionShop() { if (!gameApi()) return []; var eps = ['/api/game/player-sect/builtin-shop', '/api/game/sect/shop']; for (var ei = 0; ei < eps.length; ei++) { try { var res = await gameApi().get(eps[ei]); console.log('[pavilion] ' + eps[ei] + ' code=' + (res && res.code)); if (res && res.code === 200 && res.data) { if (Array.isArray(res.data)) return res.data; if (Array.isArray(res.data.items)) return res.data.items; for (var dk in res.data) { if (res.data.hasOwnProperty(dk) && Array.isArray(res.data[dk])) return res.data[dk]; } } } catch (_) {} } console.log('[pavilion] all failed'); return []; }
