...
适用于 `ling.muge.info` 的浏览器用户脚本，主要用于神识消耗、商人处理、护道雇佣、妖兽遭遇、自动恢复、冥想续跑、藏宝图、铭文洗练与更新公告。
...
- 藏宝图流程：支持自动试炼和自动刷藏宝图，可设置最多用几张、一次用几张和每次间隔，遇到守卫时沿用护道与自战配置；缺图、神识不足、批次上限和异常会等待重试。
- 铭文洗练：支持按页面显示的等级关键词、目标属性、最小值、命中模式和最大次数分类配置，自动十连洗练并放弃不符合目标的结果；可选择命中后自动装配，优先级为空槽位、非目标槽位、目标槽最低值且新值更高。
- 浏览器通知：发现新版本、铭文命中目标和关键异常时可弹系统通知。
- 虚空淬体：探索前自动检查加成，可按丹药等级和购买数量补充。
- 隐秘符：探索和刷藏宝图前可自动从背包尝试使用隐秘符，背包没有时按“隐秘符”从坊市购买；无法可靠检测加成时以使用接口成功为准。
- 更新公告：内置版本公告，支持读取 GitHub `release.json`，页面不刷新时也会定时检查云端新版本。
- 面板交互：顶部菜单分类、可拖拽、可收起、可调节窗口大小，设置会保存在浏览器本地。
- 面板层级：打开游戏铭文页或其他弹层时，脚本面板会保持在上层，方便继续操作。
...
- `妖兽恢复`：弱怪自战、自动回血回灵。
- `自动流程`：冥想续跑、藏宝图、虚空淬体、隐秘符。
- `铭文`：铭文洗练目标、等待间隔、最大次数和运行日志。
- `更新`：云端公告地址和手动检查更新。
...
如果只统计你自己电脑上的浏览器，不需要内网穿透；如果要让别人安装脚本后把在线心跳发到你电脑上，就需要公网可访问地址。项目里已经准备了 Cloudflare 临时隧道启动脚本：

```text
start-online-public.ps1
```

运行后会启动本机后台 `online-server.js`，生成一个 `trycloudflare.com` 公网地址，并把脚本里的 `DEFAULT_ONLINE_STATS_ENDPOINT` 改成公网心跳地址。后台页面地址会显示在命令行里，打开即可看当前在线人数。
