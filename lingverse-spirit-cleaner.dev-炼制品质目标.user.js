...
(function () {
    ...
    // --- 珍宝阁 ---
    async function fetchPavilionShop() { if (!gameApi()) return []; var eps = ['/api/game/player-sect/builtin-shop', '/api/game/sect/shop']; for (var ei = 0; ei < eps.length; ei++) { try { var res = await gameApi().get(eps[ei]); if (res && res.code === 200 && res.data) { if (Array.isArray(res.data)) return res.data; if (Array.isArray(res.data.items)) return res.data.items; if (Array.isArray(res.data.list)) return res.data.list; if (Array.isArray(res.data.shop)) return res.data.shop; } } catch (_) {} } return []; }
