...
(function () {
    ...
    async function ensureNirvanaPill() {
        ...
        var wantedRarity = state.nirvanaRarity || 3;
        var item = invRes.data.find(function(it) {
            var t = String(it.templateId || '').toLowerCase();
            if (t.indexOf('pill_nirvana_') < 0 && t.indexOf('bp_pill_rebirth') < 0) return false;
            return (it.rarity || 0) >= wantedRarity;
        });
        if (item) {
        ...
        try {
            ...
            if (nirvana) {
                ...
                if (invRes2 && invRes2.code === 200 && Array.isArray(invRes2.data)) {
                    var wantedR = state.nirvanaRarity || 3;
                    var crafted = invRes2.data.find(function(it) { var t = String(it.templateId || '').toLowerCase(); return (t.indexOf('pill_nirvana_') >= 0 || t.indexOf('bp_pill_rebirth') >= 0) && (it.rarity || 0) >= wantedR; });
                    if (crafted) {
    ...
    var state = {
        ...
        autoFullHeal: localStorage.getItem('lvSpiritCleaner.autoFullHeal') !== '0',
        autoFullHealInterval: readNumber('lvSpiritCleaner.autoFullHealInterval', 30000),
        ...
        farmAfterRefresh: localStorage.getItem('lvSpiritCleaner.farmAfterRefresh') === '1',
        disposeAfterRefresh: localStorage.getItem('lvSpiritCleaner.disposeAfterRefresh') === '1',
        washUpgradeAfterRefresh: localStorage.getItem('lvSpiritCleaner.washUpgradeAfterRefresh') === '1',
    ...
    function autoFullHeal() {
        try {
            var a = gameApi();
            if (a && typeof a.post === 'function') {
                a.post('/api/game/heal-with-mp', {hpAmount: -1});
            }
        } catch(e) {}
    }
    // 自动灵宠回血
...
function stopAutoPetHealTimer() {
    if (_autoPetHealTimer) { clearInterval(_autoPetHealTimer); _autoPetHealTimer = null; }
        ...
        function startAutoHealTimer() {
        if (_autoHealTimer) clearInterval(_autoHealTimer);
        if (!state.autoFullHeal || state.autoFullHealInterval < 1500) return;
        _autoHealTimer = setInterval(function() {
            autoFullHeal();
        }, state.autoFullHealInterval);
    }
    ...
    function readUiToState() {
        ...
        state.autoFullHeal = chk('lvscAutoFullHeal');
        state.autoFullHealInterval = Math.max(1500, num('lvscAutoFullHealInterval', 1500, 30000));
    ...
    async function tryAdvancedMeditateOnce() {
        ...
        // 15分钟冷却
        if (window._advMedCooldown && Date.now() - window._advMedCooldown < 900000) {
            return false;
        }
        // 只在夏季高级冥想 选项
    ...
    function buildPanel() {
        ...
        panel.innerHTML =
            ...
            '<div class="lvsc-section">' +
                        '<div class="lvsc-section">' +
            '<div class="lvsc-section-title-row"><span>自动运动(耗灵)</span><label class="lvsc-check"><input id="lvscAutoFullHeal" type="checkbox">开启</label></div>' +
            '<div class="lvsc-grid2">' +
            '<label>间隔(ms)<input id="lvscAutoFullHealInterval" type="number" min="1000" step="500" title="最小值1500ms(1.5秒)"></label>' +
            '</div>' +
            '<div class="lvsc-section">' +
            ...
            '</div>' +
            '<div class="lvsc-help">调用API给出站受伤灵兽回血。</div>' +
            '</div>' +
            '<div class="lvsc-help">自动调用 API 回满血，不打开界面。</div>' +
            '</div>' +
        ...
        document.getElementById('lvscAutoFullHeal').checked = state.autoFullHeal;
        document.getElementById('lvscAutoFullHealInterval').value = String(state.autoFullHealInterval);
        ...
        (function() {
            var cb = document.getElementById('lvscAutoFullHeal');
            if (cb) {
                cb.onchange = function() {
                    state.autoFullHeal = chkVal('lvscAutoFullHeal');
                    persistSetting('lvSpiritCleaner.autoFullHeal', state.autoFullHeal);
                    startAutoHealTimer();
                };
            }
        })();
        (function() {
            var el = document.getElementById('lvscAutoFullHealInterval');
            if (!el) return;
            el.oninput = function () {

                var v = Math.max(1500, Number(el.value)) || 30000;
                state.autoFullHealInterval = v;
                persistSetting('lvSpiritCleaner.autoFullHealInterval', String(v));
                startAutoHealTimer();
            };
        })();
                (function() {
        ...
        try {(function() {
            ...
            (function() {
                ...
                // 开始/停止
                cfgRow.innerHTML += '<label class="lvsc-check" style="font-size:11px"><input id="lvscDisposeAfterRefresh" type="checkbox">刷新后自启</label>';
            ...
            (function() {
                ...
                s.innerHTML += '<label class="lvsc-check" style="font-size:11px"><input id="lvscFarmAfterRefresh" type="checkbox">刷新后自启</label>';
                var btnRow = el('div'); btnRow.style.cssText = 'display:flex;gap:6px';
            ...
            (function() {
                ...
                s.innerHTML += '<label class="lvsc-check" style="font-size:11px;margin-top:2px"><input id="lvscWashUpgradeAfterRefresh" type="checkbox">刷新后自启</label>';
            ...
            setTimeout(function() {
                ...
                var farmAr = document.getElementById('lvscFarmAfterRefresh');
                if (farmAr) { farmAr.checked = state.farmAfterRefresh; farmAr.onchange = function() { state.farmAfterRefresh = this.checked; persistSetting('lvSpiritCleaner.farmAfterRefresh', this.checked); }; }
            ...
            setTimeout(refreshPavilionItems, 2000);
                ...
                var washAr = document.getElementById('lvscWashUpgradeAfterRefresh');
                if (washAr) { washAr.checked = state.washUpgradeAfterRefresh; washAr.onchange = function() { state.washUpgradeAfterRefresh = this.checked; persistSetting('lvSpiritCleaner.washUpgradeAfterRefresh', this.checked); }; }
                ...
                var dispAr = document.getElementById('lvscDisposeAfterRefresh');
                if (dispAr) { dispAr.checked = state.disposeAfterRefresh; dispAr.onchange = function() { state.disposeAfterRefresh = this.checked; persistSetting('lvSpiritCleaner.disposeAfterRefresh', this.checked); }; }
    ...
    function waitForGame() {
        if (document.body && (window.api || window._lastPlayerData || document.getElementById('exploreBtn'))) {
            ...
            // === 刷新后自启（独立于主循环）===
            if (state.farmAfterRefresh) {
                setTimeout(function() { var fb = document.getElementById('lvscFarmStartBtn'); if (fb) fb.click(); }, 3000);
            }
            if (state.disposeAfterRefresh) {
                setTimeout(function() { var db = document.getElementById('lvscDisposeStartBtn'); if (db) db.click(); }, 5000);
            }
            if (state.washUpgradeAfterRefresh) {
                setTimeout(function() { var wb = document.getElementById('lvscWashStoneUpgradeStartBtn'); if (wb) wb.click(); }, 7000);
            }
            // === 自动刷新（移到if外面，始终生效）===
